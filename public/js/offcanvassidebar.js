...
var OffCanvasSidebar = (function() {
	...
	function init() {
		...
		var container = $('.outter-wrap'),
			triggerBtn = $('#trigger-btn'),
			header = $('.header-animate.header-fixed'),
			sideBarInner = $('.sidebar-nav-inner'),
			...
			bodyClick = function(evt) {
				if( !hasParentClass( evt.target, 'sidebar-nav' ) ) {
					resetMenu();
		...
		$(triggerBtn).bind(eventtype, function(evt){
			evt.stopPropagation();
			evt.preventDefault();
			$(container).addClass('sidebar-push-effect');
			setTimeout(function(){
